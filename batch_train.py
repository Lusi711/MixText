...
def parse_argument():
    ...
    parser.add_argument('--low_resource', action='store_true', help='whther to train low resource dataset')

    args = parser.parse_args()
    if args.data == 'trec':
        assert args.label_name in ['label-fine', 'label-coarse'], \
            "If you want to train on TREC dataset with augmentation, you have to name the label of split either " \
            "'label-fine' or 'label-coarse'"
    ...
    if 'aug' in args.modes:
        assert [file for file in os.listdir(args.aug_dir) if 'times' in file], \
            'This directory has no augmentation file, please input correct aug_dir!'
...
def batch_train(args):
    for seed in args.seeds:
        for mode in args.modes:
            run_args = args
            run_args.mode, args.seed = mode, int(seed)
            run_args.epoch, run_args.batch_size, run_args.aug_batch_size, run_args.val_steps, run_args.max_length, \
            run_args.augweight = settings[args.data].values()
            if mode == 'raw':
                if args.random_mix:
                    os.system(
                        'python run.py --label_name {} --mode {}   --seed {} --data {} '
                        '--random_mix {}  --epoch {epoch} --batch_size {batch_size} --aug_batch_size {aug_batch_size} '
                        '--val_steps {val_steps} --max_length {max_length} --augweight {augweight} '.format(
                            args.label_name, mode, int(seed), args.data, args.random_mix, **settings[args.data]
                        )
                    )
                else:
                    os.system(
                        'python run.py --label_name {} --mode {}  --seed {} --data {}   '
                        '--epoch {epoch} --batch_size {batch_size} --aug_batch_size {aug_batch_size} '
                        '--val_steps {val_steps} --max_length {max_length} --augweight {augweight} '.format(
                            args.label_name, mode, int(seed), args.data, **settings[args.data]
                        )
                    )
            else:
                for aug_file in os.listdir(args.aug_dir):
                    if args.prefix:
                        ...
                        if aug_file.startswith(args.prefix):
                            aug_file_path = os.path.join(args.aug_dir, aug_file)
                            assert os.path.exists(aug_file_path)
                            os.system(
                                'python run.py --label_name {} --mode {} --seed {} --data {} --data_path {} '
                                '--epoch {epoch} --batch_size {batch_size} --aug_batch_size {aug_batch_size} '
                                '--val_steps {val_steps} --max_length {max_length} --augweight {augweight} '.format(
                                    args.label_name, mode, int(seed), args.data, aug_file_path, **settings[args.data]
                                )
                            )
                    else:
        ...
        for seed_num in os.listdir(partial_split_path):
            ...
            for mode in args.modes:
                if mode == 'raw':
                    if args.random_mix:
                        os.system(
                            'CUDA_VISIBLE_DEVICES={} python run.py --low_resource_dir {} --seed {} --output_dir {} '
                            '--label_name {} --mode {}   --data {} --random_mix {}  --epoch {epoch} '
                            '--batch_size {batch_size} --aug_batch_size {aug_batch_size} --val_steps {val_steps} '
                            '--max_length {max_length} --augweight {augweight}'.format(
                                args.GPU, partial_split_seed_path, int(seed_num.split('_')[1]), args.output_dir,
                                args.label_name, mode, args.data, args.random_mix, **settings[args.data]
                            )
                        )
                    else:
                        os.system(
                            'CUDA_VISIBLE_DEVICES={} python run.py --low_resource_dir {} --seed {} --output_dir {} '
                            '--label_name {} --mode {}  --data {}   --epoch {epoch} --batch_size {batch_size} '
                            '--aug_batch_size {aug_batch_size} --val_steps {val_steps} --max_length {max_length} '
                            '--augweight {augweight}'.format(
                                args.GPU, partial_split_seed_path, int(seed_num.split('_')[1]), args.output_dir,
                                args.label_name, mode, args.data, **settings[args.data]
                            )
                        )
                elif mode == 'raw_aug':
                    for aug_file in [file for file in os.listdir(partial_split_seed_path) if file.startswith('times')]:
                        ...
                        os.system(
                            'CUDA_VISIBLE_DEVICES={} python run.py --low_resource_dir {} --seed {} --output_dir {} '
                            '--label_name {} --mode {}  --data {} --data_path {} --epoch {epoch} '
                            '--batch_size {batch_size} --aug_batch_size {aug_batch_size} --val_steps {val_steps} '
                            '--max_length {max_length} --augweight {augweight} '.format(
                                args.GPU, partial_split_seed_path, int(seed_num.split('_')[1]), args.output_dir,
                                args.label_name, mode, args.data, aug_file_path, **settings[args.data]
                            )
                        )
...
if __name__ == '__main__':
    ...
    settings = task_settings.train_settings
    if args.low_resource:
